...
namespace {
    ...
    final class RouterTest
    {
        ...
        public function testParameterValidation(): void
        {
            ...
            // Test parameter validation - use a parameter that contains dangerous content but matches the pattern
            $testCases = [
                '/users/test..backup', // Contains ..
                '/users/' . str_repeat('a', 256), // Too long
                '/users/test%00admin', // Contains null byte (URL encoded)
            ];

            foreach ($testCases as $testPath) {
                if ($routeInfo->matches('GET', $testPath)) {
                    try {
                        $routeInfo->extractParams($testPath);
                        // If we get here without exception, this test case didn't work
                        continue;
            ...
            // If none of the test cases worked, fail the test
            $this->fail("No InvalidArgumentException was thrown for any dangerous parameter");
        }
        ...
        public function testPatternDebugging(): void
        {
            $routeInfo = RouteInfo::fromPath('GET', '/users/{id}', 'App\\Actions\\TestAction');

            // Debug ausgabe
            echo "\nOriginal path: /users/{id}\n";
            echo "Generated pattern: " . $routeInfo->pattern . "\n";
            echo "Parameter names: " . implode(', ', $routeInfo->paramNames) . "\n";

            // Test verschiedene Pfade
            $testPaths = ['/users/123', '/users/abc', '/users/', '/posts/123'];
            foreach ($testPaths as $testPath) {
                $matches = $routeInfo->matches('GET', $testPath);
                echo "Path '{$testPath}' matches: " . ($matches ? 'YES' : 'NO') . "\n";
            }
    ...
    class MockContainer implements \Framework\Container\ContainerInterface
    {
        ...
        public function get(string $id): mixed
        {
            ...
            if ($id === 'App\\Actions\\TestAction') {
                return new \App\Actions\TestAction();
            }
        ...
        public function has(string $id): bool
        {
            return isset($this->services[$id]) || $id === 'App\\Actions\\TestAction';
        }
